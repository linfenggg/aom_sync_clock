...
set rc [catch {
  open_project {MASTER_FPGA.prj}
  import_device ph1_60.db -package PH1A60GEG324
  elaborate -top {top}
  export_db {MASTER_FPGA_elaborate.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  read_adc ../../PH1A_TOP.adc
  optimize_rtl
  report_area -file MASTER_FPGA_rtl.area
  export_db {MASTER_FPGA_rtl.db}
} RESULT]
if {$rc} {
  step_error opt_rtl
  return -code error $RESULT
...
set rc [catch {
  ...
  optimize_gate -maparea MASTER_FPGA_gate.area
  legalize_phy_inst
  export_db {MASTER_FPGA_gate.db}
} RESULT]
if {$rc} {
  step_error opt_gate
  return -code error $RESULT
